...
func createPasiens(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "POST" {
		...
		stmt, err := db.Prepare("INSERT INTO pasiens (No_ktp,Nama,Jenis_kelamin,Alamat,Status,Lama_menginap) VALUES (?,?,?,?,?,?)")
...
func getPasien1(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	var pasiens []Pasien
	params := mux.Vars(r)
...
func updatePasiens(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "PUT" {
		...
		stmt, err := db.Prepare("UPDATE pasiens SET Nama = ?, Jenis_kelamin = ?, Alamat = ?, Status = ?, Lama_menginap = ? WHERE No_ktp = ?")
...
func delPasiens(w http.ResponseWriter, r *http.Request) {
	...
	stmt, err := db.Prepare("DELETE FROM pasiens WHERE No_ktp = ? AND Nama = ?")
...
func main() {
	...
	r.HandleFunc("/pasiens/{id}", getPasien1).Methods("GET")
